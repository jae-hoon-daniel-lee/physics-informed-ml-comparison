...
'''
Computes the theoretically derived discrete gradient.
'''
class discrete_differential(torch.autograd.Function):
    ...
    def forward(ctx, x1, x2, v1, v2):
        '''
        g = (f(x1) - f(x2)) / (x1 - x2) = ∇(bar)f(x1, x2)
        '''
        # v1 = f(x1), v2 = f(x2)
        dx = x1 - x2  # dx = x1 - x2
        dv = v1 - v2  # dv = f(x1) - f(x2)
        g = dv / dx   # g = (f(x1) - f(x2)) / (x1 - x2)
        ctx.save_for_backward(dx, dv, g)
        return g      # g: discrete gradient value

    @staticmethod
    def backward(ctx, grad):
        '''
        g = (v1 - v2) / (x1 - x2)
        ∂g/∂x1=-(v1-v2)/(x1-x2)^2=-g/dx, etc. 
        '''
        dx, dv, g = ctx.saved_tensors
        gv1 = 1. / dx  # gv1 = d(g)/d(v1)
        gv2 = -gv1     # gv2 = d(g)/d(v2)
        gx2 = g / dx   # gx2 = d(g)/d(x2)
        gx1 = -gx2     # gx1 = d(g)/d(x1)
        return grad * gx1, grad * gx2, grad * gv1, grad * gv2

'''
Defines forward/backward for the discrete nonlinear module.
'''
class apply_discrete_differential(torch.autograd.Function):
    ...
    def backward(ctx, grad_v1, grad_v2):
        '''
        Computes the derivative wih respect to the input x1 and x2
        using grad_v1 and grad_v2
        '''
        x1, x2, v1, v2 = ctx.saved_tensors
        # backward path can be discrete version or not
        if discrete_autograd_mode:
            '''
            This mode is activated when we predict or need discrete gradient strictly.

            - Objective:
                Computes the discrete gradient ∇_bar H 
                of the learned Hamiltonian function H(u)

            - How:
                1) Computes ∇_bar f, which is the discrete gradient of the actiavtion function 
                   with respect to the two inputs u^(n) and  u^(n+1).
                   This ∇_bar f is used for prediction of the next state of the dynamic system.
                2) Then, computes ∇_bar H using ∇_bar f

            - Implication:
                ∇(bar)H computation for the solution of discrete ODE
                   du/dt=G(bar)∇(bar)H
            '''
            if eps is None:
                set_eps()
            ...
            '''
            Loss: L = || Δu/Δt - G(bar) ∇(bar) H ||^2 = L (w; u1, u2) 
            
            d(bar) H = (H(u2) - H(u1)/ Δu) Δu = ∇(bar) H Δu 
            
            ∇(bar) H = H(u2) - H(u1)/ Δu 
            
            ∂L/∂w = ∂L/∂H |_{u=u1} ∂H/∂w |_{u=u1} + ∂L/∂H |_{u=u2} ∂H/∂w |_{u=u2} 
            
            H: = H(w; u) = F(w) u 
            
            ∂L/∂w = ∂L/∂H|_{u=u1} u_1^T ∂F/∂w + ∂L/∂H|_{u=u2} u_2^T ∂F/∂w 
                  = (∂L/∂H|_{u=u1} u_1^T + ∂L/∂H|_{u=u2} u_2^T) ∂F/∂w 

            In the situation where NN input at u1, u2 and ouput play, 
            w is updated at once with ∂F/∂w. 
            '''
            grad_x1 = ctx.grad_fn(x1, v1)
            grad_x2 = ctx.grad_fn(x2, v2)
            '''
            backpropagation chain rule:
              - ∂L/∂v1,2: gradient passed-down though the upper layer
              - ∂L/∂x1,2: what we need for the gradient at x1, x2

                  ∂L/∂x1,2 = ∂L/∂v1,2 x ∂v1,2/∂x

            ∂L/∂x1,2: the derivative with respect to 
                      the weight:=w dependence-included 
...
def DiscreteDifferentialLinearDecorator(cls):
    ...
    class DecoratedLinearModule(cls):
        ...
        '''
        linear function: f(x) = Ax + b (A: weight matrix, b: bias)
        discrete gradient: ∇(bar) f(x1,x2)=f(x2)-f(x1)/(x2-x1)
           --> ((Ax2+b)-(Ax1+b))/(x2-x1)=A(x2-x1)/(x2-x1)=A
        continuous derivative: f'(x)=A
        '''
...
def DiscreteDifferentialNonlinearDecorator(cls, func=None, grad_fn=None):
    ...
    class DecoratedNonlinearModule(cls):
        ...
        def forward(self, x1, x2=None):
            if x2 is None:
                # case only one input: excutes the usual prediction or NN output
                forward = super(DecoratedNonlinearModule, self).forward
                return forward(x1)
            
            # case two inputs (i.e., train or discrete auto gradient mode):
            # executes spectial forward and backprop (discrete gradient compution)            # 특별한 순전파 및 역전파(이산 기울기 계산)를 수행합니다.
